...
| **T2** | Head — physics object: rolls down the entry slope, **stops at the snag**, `release()` lets it roll out the exit slope. Signals: `stuck`, `released`, `left_scene`. *RigidBody2D (circle) · snag = a StaticBody2D/obstacle or `freeze`* | Sean / Ben | D2 ✔ | M | head rolls in, stops, `release()` rolls it out of the scene, every run |
...
| **T8** | **Day template scene** — entry slope · flat middle · exit slope · head spawn · body spawn · Sun · HUD · WinConditions container · camera. Plus a 5-line "how to make a new day" note in the scene/README (one file per day) | Sean / Ben | T1–T7 | M | a new day is one new file that runs end to end with no edits elsewhere |
