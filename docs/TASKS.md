...
| **T2** | Head — **starts already stuck** at its authored position; `release()` rolls it off screen (translate + spin, scripted). Signals: `released`, `left_scene`. Joins group `"head"` so `Game` can find it without a hard path. *RigidBody2D held `freeze`d (set in `head.tscn`) and moved by script — scripted, not simulated (DESIGN §2.1)* **Scope cut Fri 18:45:** no roll-in, no entry/exit slopes, no `stuck` signal (it starts stuck). **Fri 19:15:** the roll off screen is now the day's whole outro beat — no separate outro scene (DESIGN §2.1 "Day end / outro"). | Sean / Ben | D2 ✔ | S | head sits still at open; `release()` sends it off screen and emits `left_scene`, every run |
...
| **T4** | Keyed win-condition node — `WinCondition` with `@export key` (`body` / `mind`), `satisfy()`, `satisfied` signal, plus a `WinConditions` manager node that finds every condition in the day and emits `Events.day_completed` when the last one lands. `Game` is what calls `head.release()` off that signal, not this node. | Sean / Ben | — | M | two dummy WinConditions satisfied in either order → head releases; one alone → nothing |
| **T5** | Day flow — `Game` autoload is the scene manager: `DAY_SCENES` list, `start_days()`, `next_day()`, `restart_day()`, `go_to(i)` for testing; listens to `Events.day_completed`, releases the head, and advances only once `head.left_scene` fires; after the last day → reunion scene. Sole owner of `change_scene_to_file()`. | Sean / Ben | — | S–M | a stub day chains to itself twice and then to a "reunion" placeholder |
...
| **C2** | **Day 1** — the minimum ship. Pick the first day card (from the brainstorm needs) and build it on the template | ? | T8, one card | M | slope in → snag → both needs → head rolls out → next day, in the web build |
