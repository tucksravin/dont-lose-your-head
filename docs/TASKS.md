...
| **T3** | Sun timer — `@export day_length`, sprite arcs across the top, emits `sunset` → **day fails → restart**. *`scenes/sun/sun.tscn`, `_process` on a sine arc* | Sean / Ben | D5 (default ok) | S | **half done:** the sun arcs and emits `sunset` — nothing listens yet. Done when the day restarts at sunset (and a won day ignores it) |
