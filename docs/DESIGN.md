...
| **Don't Panic** *(rework, smahr Sat 14:50 — thoughts.md)* | Head hangs in a cage in the air. Moving winds **panic** (fail at max). Kikis fly out of the cage (hit = fail). Stand on the floor button and `interact` to open it — that satisfies **body + mind**. File: `scenes/days/day_panic.tscn`. **No use key (Sat, Tucker): stepping onto the floor button frees the cage; standing on it still puts the body in `kiki_safe`.** |
| **(Velma)** *(built, not yet human-playtested — see [days/velma-plan.md](days/velma-plan.md))* | **Velma** (`day_velma`). Head sits near the entrance without glasses (`Game.wearing_glasses` is false after `transition_glasses`). `glasses.png` sits atop a 4-step climb **later in the room** (same 36 px one-way-step pattern). Scene is darkened (`CanvasModulate` + dusk sky) so the glasses aren't readable from the start. Reach them = **body**; carry back to the head = **mind** and `Head.set_wearing_glasses(true)`. Vision blur follows the body. File: `scenes/days/day_velma.tscn`. **Slot:** cage → panic → glasses transition → velma → rest of the run keeps glasses. |
