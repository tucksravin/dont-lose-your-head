...
| **Lockdown** *(C3b, smahr Sat 11:02)* | WarioWare split attention. Setup beat: head waits alone in the middle; interact picks it up, a vertical bar blocks the right exit, a pedestal opens on the right; interact seats the head. Then the puzzle: dodge a dense, fast rain of thoughts (hit → game-over card) while answering 4 glance-math puzzles on **3 labeled floor pads** with **interact** (E). Wrong pad or a hit → card. Last correct answer **tips the pedestal toward the exit** and the head rolls off. File: `scenes/days/day_lockdown.tscn`. **No use key (Sat, Tucker): walking up to the head picks it up, standing on the pedestal seats it, and an answer is chosen by LANDING on its plate — the plates shrank to 32×8 so you can jump the ones you don't want. Walking across a plate is deliberately safe; landing on a wrong one still fails the day.** |
| **Working Out** *(C3c, smahr Sat 12:01)* | Button masher. Loose head on the right, kikis swarm it (`kiki_frames.tres`) and **block the walk to the head**. Body picks up a placeholder barbell (`interact`), then mashes `jump` (Space) — the bar pumps up and down — to push the swarm out. Kikis **creep back**. Fail: **sunset** or **kikis touching the head**. Swarm gone satisfies body + mind; head releases. File: `scenes/days/day_workout.tscn`. **No use key (Sat, Tucker): walking into the barbell picks it up.** |
| **Don't Panic (still)** *(smahr Sat 15:18, cliff Sat 15:23, tree Sat 15:30)* | Head caged **in a tree** on the right-edge cliff (palette placeholder — no tree sprite yet). Moving winds **panic**; stand still until it hits **0**. Then the head **sighs and falls backward** out of the canopy and off the drop → `transition_cage`. Fail at max or walking off the cliff. Kiki cloud closes in as panic rises. File: `scenes/days/day_panic_still.tscn`. **No use key anywhere in the game (Sat, Tucker) — this day never had one.** |
| **Don't Panic (cage)** *(rework, smahr Sat 14:50 — thoughts.md)* | Head hangs in a cage in the air. Moving winds **panic** (fail at max). Flying kikis to jump (hit = fail). Stand on the floor button and `interact` to open it — that satisfies **body + mind**. Same kiki cloud as the still day. File: `scenes/days/day_panic.tscn`. Slot: still → cage transition → this → glasses transition. **No use key (Sat, Tucker): stepping onto the floor button frees the cage; standing on it still puts the body in `kiki_safe`.** |
| **(Velma)** *(built, not yet human-playtested — see [days/velma-plan.md](days/velma-plan.md))* | **Velma** (`day_velma`). Head sits near the entrance without glasses (`Game.wearing_glasses` is false after `transition_glasses`). `glasses.png` sits atop a 4-step climb **later in the room** (same 36 px one-way-step pattern). Scene is darkened (`CanvasModulate` + dusk sky) so the glasses aren't readable from the start. Reach them = **body**; carry back to the head = **mind** and `Head.set_wearing_glasses(true)`. Vision blur follows the body. File: `scenes/days/day_velma.tscn`. **Slot:** still panic → cage transition → hanging panic → glasses transition → velma → rest of the run keeps glasses. |
