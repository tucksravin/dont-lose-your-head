...
| **Working Out** *(C3c, smahr Sat 12:01)* | Button masher. Loose head on the right, kikis swarm it (`kiki_frames.tres`) and **block the walk to the head**. Body picks up a placeholder barbell (`interact`), then mashes `jump` (Space) — the bar pumps up and down — to push the swarm out. Kikis **creep back**. Fail: **sunset** or **kikis touching the head**. Swarm gone satisfies body + mind; head releases. File: `scenes/days/day_workout.tscn`. |
| **(Velma)** *(built, not yet human-playtested — see [days/velma-plan.md](days/velma-plan.md))* | **Velma** (`day_velma`). Head opens already without its glasses, which sit atop a 4-step platform climb (same 36 px one-way-step pattern as Bridge/C2's stair). Reaching them satisfies the **body** need and they become carried (follow the body); climbing back down and walking them within reach of the head satisfies the **mind** need ("handed over") and they disappear. A real screen-space blur (`VisionBlur`, a `canvas_item` shader reading `SCREEN_TEXTURE`) follows the body around a small sharp circle — the head's blurry vision, shared with the body per the game's thesis — and shrinks further the faster the body moves, then fades out once the glasses are delivered. ~30 s. File: `scenes/days/day_velma.tscn`; new reusable nodes: `scenes/gameplay/glasses.tscn`, `scenes/gameplay/vision_blur.tscn` (+ `vision_blur.gdshader`, the project's first shader). Open calls (not locked): whether the glasses fall in a future intro beat, and this day's final slot in `Game.DAY_SCENES` (currently first) — see the plan doc §4. |
| **Mirror World** *(C3d, smahr Sat 13:49)* | Head on a right-hand platform, staring into a tall glass from floor to skull. It **alternates** look-at-glass (walk *and* jump flipped: ↓ hops) and look-at-player (normal). Kikis fly out of the glass toward the body — jump them; a hit is the game-over card. Walk to the glass, `interact` throws it off the right; the head rolls after it. Fail: **sunset** or **kiki**. File: `scenes/days/day_mirror.tscn`. |
