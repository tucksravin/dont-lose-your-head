...
| **Don't Panic** *(rework, smahr Sat 14:50 — thoughts.md)* | Head hangs in a cage in the air. Moving winds **panic** (fail at max). Kikis fly out of the cage (hit = fail). Stand on the floor button and `interact` to open it — that satisfies **body + mind**. File: `scenes/days/day_panic.tscn`. |
