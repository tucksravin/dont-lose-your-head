...
| **Don't Panic (still)** *(smahr Sat 15:18; uncaged Sat 17:30; tint+shake Sat 20:27)* | Head **stuck in a tree** on the right-edge cliff — **not caged** (the cage drops in `transition_cage`). Moving winds **panic**; stand still until it hits **0**. The head **greens and shakes** as the meter rises (same `set_panic_ratio` / `set_agitation` as the cage). Then it **sighs and falls backward** out of the canopy and off the drop. Fail at max or walking off the cliff. Kiki cloud closes in as panic rises. File: `scenes/days/day_panic_still.tscn`. |
