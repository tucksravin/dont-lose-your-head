...
- **Kiki** — [kiki.tscn](../scenes/gameplay/kiki.tscn) / [kiki.gd](../scenes/gameplay/kiki.gd) (`class_name Kiki`, an `AnimatedSprite2D`): **one intrusive thought, self-scrambling.** Every kiki picks a **random start frame** (plus a random sub-frame phase, so two on the same frame drift apart) and a **random right-angle rotation** in `_ready()`, so no two look alike (Tucker, Sat). `kind` is `lil_kiki` (9 frames) or `big_kiki` (12) — the two loops in `kiki_frames.tres`. Place it in the editor (pick `kind` in the Inspector) or spawn it from code: `Kiki.spawn(parent, &"big_kiki", Vector2(320, 90))` returns the node (`Kiki.make()` = same, no `add_child`); `scramble()` re-rolls; `scramble_on_ready = false` keeps what the editor saved. An unknown `kind` warns and falls back to lil. **Not yet used by** [kiki_swarm.gd](../scenes/gameplay/kiki_swarm.gd) / [flying_kiki.gd](../scenes/gameplay/flying_kiki.gd) (smahre's — they build their own sprites); swapping them onto `Kiki` is ~2 lines each, their call.
...
| [day_panic.tscn](../scenes/days/day_panic.tscn) + [.gd](../scenes/days/day_panic.gd) | [1] | DayManager | E on the floor button (body+mind); fail = panic max or kiki or sunset | `caged = true` hanging at (520,168), glasses on | "E at the button frees the cage…" |
...
| [day_lockdown.tscn](../scenes/days/day_lockdown.tscn) | [5] | DayManager | setup (carry head to pedestal) then body+mind via PuzzleChain (4 pad puzzles, `interact`); ThoughtRain fails on hit (fast, burst 2); win tips the pedestal | loose in the middle (320,306), seated at (560,258) | "Walk to the head and press E." then the puzzle line |
| [day_workout.tscn](../scenes/days/day_workout.tscn) | [6] | DayManager | barbell pickup then mash `jump` vs KikiSwarm (creep-back, pumps bar); kiki circle blocks the head; fail sunset or `kiki` | loose at (560,306) | "Walk to the barbell and press E." then mash Space |
| [day_mirror.tscn](../scenes/days/day_mirror.tscn) | [7] | DayManager | jump kikis from the glass; `interact` throws it; look-at-glass inverts walk + jump (↓ hops); fail sunset or `kiki` | on platform at (572,258) | "Jump the thoughts. E at the glass throws it. Arrows flip with the head." |
