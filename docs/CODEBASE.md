...
flowchart LR
  ...
  Dv -- "win" --> D2["platforming_day.tscn<br/>(three steps, bridge, gap)"]
...
- **PuzzleChain + AnswerPad + ThoughtRain** — Lockdown's needs: [puzzle_chain.tscn](../scenes/gameplay/puzzle_chain.tscn) (two `WinCondition` children; last correct pad satisfies body + mind; `start()` after the setup beat), [answer_pad.tscn](../scenes/gameplay/answer_pad.tscn) (`class_name AnswerPad`, group `answer_pad`; **32×8 plate — small enough to jump over; you choose it by LANDING on it**, no key: it listens to the body's `landed` signal and checks the body came down within `land_tolerance` 16 px of its centre, so walking across a plate is always safe; hidden until seated), [falling_thought.tscn](../scenes/gameplay/falling_thought.tscn) + [thought_rain.gd](../scenes/gameplay/thought_rain.gd) (hit → `fail("hit")`; `autostart` off until seated; lockdown instance `fall_speed 320`, `interval 0.32`, `burst 2`). Setup lives on [day_lockdown.gd](../scenes/days/day_lockdown.gd) (pick up → `head.attach(body)` → bar + pedestal → `detach()` + seat). Win: `_before_head_release()` tips the pedestal +90° (clockwise, toward the exit), then `head.release()`.
- **Barbell + KikiSwarm** — Workout's needs: [barbell.tscn](../scenes/gameplay/barbell.tscn) (**walk into it to pick it up**, no key; follows the body, `pump()` on mash), [kiki_swarm.tscn](../scenes/gameplay/kiki_swarm.tscn) (two `WinCondition` children; ring built with `Kiki.spawn()` so each one starts on its own frame and rotation; pressure 0 satisfies body+mind, 1 → `fail("kiki")`; creep after `begin()`; `StaticBody2D` circle blocks the walk to the head). Director: [day_workout.gd](../scenes/days/day_workout.gd).
- **Kiki** — [kiki.tscn](../scenes/gameplay/kiki.tscn) / [kiki.gd](../scenes/gameplay/kiki.gd) (`class_name Kiki`, an `AnimatedSprite2D`): **one intrusive thought, self-scrambling.** Every kiki picks a **random start frame** (plus a random sub-frame phase, so two on the same frame drift apart) and a **random right-angle rotation** in `_ready()`, so no two look alike (Tucker, Sat). `kind` is `lil_kiki` (9 frames) or `big_kiki` (12) — the two loops in `kiki_frames.tres`. Place it in the editor (pick `kind` in the Inspector) or spawn it from code: `Kiki.spawn(parent, &"big_kiki", Vector2(320, 90))` returns the node (`Kiki.make()` = same, no `add_child`); `scramble()` re-rolls; `scramble_on_ready = false` keeps what the editor saved. An unknown `kind` warns and falls back to lil. **Every kiki in the game comes from here** (Sat, Tucker): [kiki_cloud.gd](../scenes/gameplay/kiki_cloud.gd) and [kiki_swarm.gd](../scenes/gameplay/kiki_swarm.gd) spawn theirs with `Kiki.spawn()`, [flying_kiki.tscn](../scenes/gameplay/flying_kiki.tscn)'s `$Visual` carries the script, and [falling_thought.tscn](../scenes/gameplay/falling_thought.tscn)'s brown placeholder rectangle is now a kiki too — so the lockdown day rains actual intrusive thoughts.
