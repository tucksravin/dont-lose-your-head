...
[transition.tscn](../scenes/transition/transition.tscn) / [transition.gd](../scenes/transition/transition.gd): **two slopes with a shelf between** — slope 1 (−40,80)→(240,200), shelf to (320,200), slope 2 to (740,340) (one `StaticBody2D` + `CollisionPolygon2D`). The head (an `AnimatedSprite2D` on head_frames — *not* head.tscn; a frozen RigidBody2D under a PathFollow2D fights it) rides a `Path2D`/`PathFollow2D` 15 px above the ground from x=140 to x=560 (Tween on `progress`; sprite rotates by distance ÷ radius = real rolling). **Timeline:** roll 1 (`roll1_time` 1.1 s) to rest on the shelf at `pause_x` 290 → waits until the body is within `wake_distance` 72 px (or `wake_wait` 4 s) — the "I almost caught up to you" beat — `head_woke` → roll 2 (`roll2_time` 1.0 s + `brake_time` 0.3 s) to the end of the path → `_play_arrival()` when the body is within `arrive_distance` 40 px or after `arrival_wait` 2.5 s (**0 = at once**; the cage fork sets 0 so the cage lands before the body arrives) → ends only once the body has reached the head (controls off via `is_scripted`, settles; `body_arrived`) → `hold_after_arrival` 0.6 s → `fade_duration` 0.4 s → `Game.next_day()`. **The body is the player's** (Sat 08:40 — body.gd untouched; the scene only sets `floor_snap_length` = `body_floor_snap` 8). ~5–6 s if the player runs straight; open-ended if not (`HUD/Instruction` says to go after it; placeholder copy).
