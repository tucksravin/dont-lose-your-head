...
- **Mirror World** — two `WinCondition` children on [day_mirror.tscn](../scenes/days/day_mirror.tscn); director [day_mirror.gd](../scenes/days/day_mirror.gd) toggles `Head.look` + `Body.move_sign` + `invert_vertical`. [flying_kiki.tscn](../scenes/gameplay/flying_kiki.tscn) flies out of the glass (hit → `fail("kiki")`). **getting the body within `grab_radius` 56 of the glass** satisfies both (no key — Sat, Tucker); `_before_head_release()` throws the glass, then DayManager `release()`s the head after it. Reflection uses the glasses look frames when `Game.wearing_glasses`.
...
| [day_panic_still.tscn](../scenes/days/day_panic_still.tscn) + [.gd](../scenes/days/day_panic_still.gd) | [0] | DayManager | stand still until panic hits 0 (body+mind); fail = panic max, pit, or sunset; kiki cloud | `caged = true` in the tree canopy (600,168); sighs and falls off the cliff | "Hold still. Panic to zero…" |
...
| [day_mirror.tscn](../scenes/days/day_mirror.tscn) | [8] | DayManager | jump kikis from the glass; reaching the glass (`grab_radius` 56) throws it; look-at-glass inverts walk + jump (↓ hops); fail sunset or `kiki` | on platform at (572,258) | "Jump the thoughts. E at the glass throws it. Arrows flip with the head." |
