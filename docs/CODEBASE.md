...
flowchart LR
  ...
  T2 -- "Game.next_day()" --> Dv["day_velma.tscn<br/>(find glasses, put them on)"]
...
- **Glasses** — [glasses.tscn](../scenes/gameplay/glasses.tscn) / [.gd](../scenes/gameplay/glasses.gd) (`class_name Glasses`): Velma pickup (`glasses.png`). Overlap satisfies **body** and starts a carry; within `delivery_radius` of the head satisfies **mind**, hides the sprite, and `Head.set_wearing_glasses(true)`. `_ready` forces `Game.wearing_glasses = false` so an F6 on Velma still opens without them.
...
| [day_velma.tscn](../scenes/days/day_velma.tscn) | [4] | DayManager | climb later platforms to `glasses.png`, carry back to the head | loose near entrance (180,306), no glasses until delivery | "Climb up for the glasses, then bring them to your head." |
<<<<<<< HEAD
...
| [day_lockdown.tscn](../scenes/days/day_lockdown.tscn) | [6] | DayManager | setup (carry head to pedestal) then body+mind via PuzzleChain (4 pad puzzles, `interact`); ThoughtRain fails on hit (fast, burst 2); win tips the pedestal | loose in the middle (320,306), seated at (560,258) | "Walk to the head and press E." then the puzzle line |
...
| [day_mirror.tscn](../scenes/days/day_mirror.tscn) | [8] | DayManager | jump kikis from the glass; `interact` throws it; look-at-glass inverts walk + jump (↓ hops); fail sunset or `kiki` | on platform at (572,258) | "Jump the thoughts. E at the glass throws it. Arrows flip with the head." |
=======
...
| [day_lockdown.tscn](../scenes/days/day_lockdown.tscn) | [6] | DayManager | setup (walk to the head → it is picked up; stand on the pedestal → it is seated) then body+mind via PuzzleChain (4 puzzles, **hop onto the right plate**); ThoughtRain fails on hit (fast, burst 2); win tips the pedestal | loose in the middle (320,306), seated at (560,258) | "Walk to the head and press E." then the puzzle line |
| [day_workout.tscn](../scenes/days/day_workout.tscn) | [7] | DayManager | walk into the barbell to pick it up, then mash `jump` vs KikiSwarm (creep-back, pumps bar); kiki circle blocks the head; fail sunset or `kiki` | loose at (560,306) | "Walk to the barbell and press E." then mash Space |
| [day_mirror.tscn](../scenes/days/day_mirror.tscn) | [8] | DayManager | jump kikis from the glass; **reaching the glass (`grab_radius` 56) throws it**, no key; look-at-glass inverts walk + jump (↓ hops); fail sunset or `kiki` | on platform at (572,258) | "Jump the thoughts. E at the glass throws it. Arrows flip with the head." |
>>>>>>> 7d492a2038623bd9cdfbdd370b943a2f2cd53fd4
