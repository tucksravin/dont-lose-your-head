...
Godot **4.7.1**, GDScript with static typing, **640×360** pixel art, exported to the web. One run = **title → intro → the scenes in `Game.DAY_SCENES` → reunion**. Parts find each other by **signals and groups**, never by long node paths, and five autoloads hold the glue.
...
| make a transition into a day | inherit [scenes/transition/transition.tscn](../scenes/transition/transition.tscn), override `_play_arrival()` (what knocks the head loose) and/or `_play_exit()` (what happens once you catch up) — §6.2 |
| trigger something when the player reaches it | no key — fire it from the proximity/area check itself; **there is no `interact` (E) mechanic any more** — §5 |
| put a kiki (intrusive thought) in a scene | drag [scenes/gameplay/kiki.tscn](../scenes/gameplay/kiki.tscn) in, or `Kiki.spawn(self, &"big_kiki", pos)` — §5 |
...
```mermaid
flowchart LR
  Ti["title.tscn<br/>(landing screen, boot, no kikis)"] -- "button" --> I["intro.tscn<br/>(kikis panic the head off, then you chase)"]
  I -- "Game.start_days()" --> T0["transition_bird.tscn<br/>(still chasing; a bird takes it)"]
  T0 -- "Game.next_day()" --> D0["day_panic_still.tscn<br/>(tree on cliff, stand still, fall)"]
  D0 -- "win" --> T1["transition_cage.tscn<br/>(cage drops, roll off)"]
  T1 -- "Game.next_day()" --> D1["day_panic.tscn<br/>(air cage, button, kikis)"]
  D1 -- "win" --> T2["transition_glasses.tscn<br/>(roll through, glasses fly off)"]
  T2 -- "Game.next_day()" --> Dv["day_velma.tscn<br/>(climb, throw glasses)"]
  Dv -- "win" --> D2["platforming_day.tscn<br/>(ride, stomp kiki, drawbridge)"]
  D2 -- "win" --> Dlock["day_lockdown.tscn<br/>(dodge + pad puzzles)"]
  Dlock -- "win" --> Dwork["day_workout.tscn<br/>(mash kikis off the head)"]
  Dwork -- "win" --> Dmir["day_mirror.tscn<br/>(look flips left/right)"]
  Dmir -- "DayManager → Game.next_day" --> R["reunion.tscn<br/>(upside-down walk-in, dive, right, sunset)"]
  R -- "next_scene" --> M["main.tscn<br/>(placeholder end)"]
```

**Boot.** `project.godot` → `run/main_scene = scenes/ui/title.tscn` (moved off the intro Sat, `tucker/opening`). The landing screen ([scenes/ui/title.gd](../scenes/ui/title.gd)) is the palette title card — *"Let the Intrusive Thoughts In"* — with the real body + head walking in place behind it (`is_scripted` body, `Head.attach()`); its button loads the intro. The landing screen deliberately has **no kikis** — `head.tscn` carries a `HeadThoughts` stream, and `title.gd` strips it from this one head, so the first intrusive thought anyone sees is one click later. The intro ([scenes/intro/intro.gd](../scenes/intro/intro.gd)) is half cutscene, half chase: **no input for the first ~8 s** — he walks to the crest of a slope, a KikiCloud and a PanicCounter wind the head up, it pops off and rolls away — and then it clears `is_scripted` and **hands the controls over** (`Instruction` appears with the game's only controls prompt). The scene ends when the *body* has run off the right edge after the head, ~12 s in, and calls `Game.start_days()`.

**The list.** `Game.DAY_SCENES` ([scripts/autoload/game.gd](../scripts/autoload/game.gd)) is the run: `[transition_bird, day_panic_still, transition_cage, day_panic, day_workout, transition_glasses, day_velma, day_lockdown, day_mirror]`, then `Game.REUNION_SCENE`. `go_to(i)` loads entry *i* (or the reunion past the end); `next_day()` advances — and if the current scene was opened straight from the editor (F6), it first looks up where that scene sits, so testing one day still goes on to the right next one. `Game.wearing_glasses` is the only cross-scene flag: intro starts true, glasses transition sets false, Velma delivery sets true.
...
| [scenes/transition/](../scenes/transition/) | the head-gets-knocked-downhill beat + the cage / glasses / bird forks — §6.2 | night/transition |
...
**Body** — [scenes/body/body.tscn](../scenes/body/body.tscn) / [body.gd](../scenes/body/body.gd). `CharacterBody2D`, 24×32 collision, origin at the **feet**; `Visual` is an `AnimatedSprite2D` on [assets/sprites/body_frames.tres](../assets/sprites/body_frames.tres) (`idle`, `walk`, `throw`; `throw` unused) at 2×, `centered = false`, `offset (-16,-32)`. Exports `speed 150`, `jump_velocity -300`, `gravity 980`, `move_sign 1` (set to −1 to swap left/right; mirror day), `invert_vertical` (jump reads `move_down` / ↓) → **max rise 45.9 px, hang 0.61 s, 92 px reach**. Signals `jumped`, `landed`. `play_throw()` plays the `throw` sheet (mirror day; `_process` will not overwrite it). `is_scripted = true` hands control to a cutscene (intro, transition) which must then drive velocity + `move_and_slide()` itself. Group `body`. Adds a `WallLeft` `WorldBoundaryShape2D` (normal +x) on the current scene if one is missing, so the body cannot walk off the left of the fixed camera. Child `Juice` ([body_juice.gd](../scenes/body/body_juice.gd)): breathing, stretch/squash on `jumped`/`landed`, lean — scale/rotation only, `reset()` for cutscenes; delete the node and the body is as before.

**Head** — [scenes/head/head.tscn](../scenes/head/head.tscn) / [head.gd](../scenes/head/head.gd) (`class_name Head`). A **frozen** `RigidBody2D` (kinematic freeze — scripted, not simulated, DESIGN §2.1), 28×28 collision centred — **solid to the body**. Child [head_thoughts.tscn](../scenes/head/head_thoughts.tscn) (`class_name HeadThoughts`): visual-only lil kikis rise off the skull and fade (`Kiki.spawn`, Tween) — not a hazard. The same scene is instanced on the intro `HeadBlob` and the transition (export `follow` tracks the path sprite). `Visual` on [head_frames.tres](../assets/sprites/head_frames.tres): `loose` / `glasses`, `look_left` / `glasses_look_left`, `look_right` / `glasses_look_right`, `imprisoned` / `imprisoned_glasses`, `wink`. `refresh_face()` / `look()` / `release()` pick the pair from `Game.wearing_glasses`. Exports `exit_speed 180`, `exit_direction RIGHT`, `spin_speed 360`, `exit_margin 64`, `caged` (plays the cage loop), `jitter_px`/`jitter_full_agitation` (*anim*: sprite-only shake). `release()` (idempotent) → translates + spins off screen → `left_scene`. `look(dir)` plays look_left (−1) / look_right (+1) / loose (0); no-op if caged. `attach(carrier, offset)` / `detach()` — follow a node without reparenting (lockdown carry); collision off while attached; `offset.x` flips with the carrier's `Visual.flip_h`. `set_solid(bool)`. Signals `released`, `left_scene`. `set_agitation(x)` scales the cage loop speed (and the shake). Group `head`. Placement: y = 306 sits on a y = 320 floor.
...
- **PanicCounter** — [panic_counter.tscn](../scenes/gameplay/panic_counter.tscn) / [.gd](../scenes/gameplay/panic_counter.gd) (`class_name`): panic starts at 15, rises while the body moves, holds, then falls when still; **max fails** the day (`DayManager.fail("panic")`). `win_on_zero` (off by default) satisfies every WinCondition child at 0 — on in [day_panic_still.tscn](../scenes/days/day_panic_still.tscn), off in the hanging-cage day (the button wins). Drives `head.set_agitation()` / `set_panic_level()` (the heartbeat lives on the head, not Sfx); signals `panic_changed(int)`, `calmed`; group `panic_counter`. [panic_label.gd](../scenes/gameplay/panic_label.gd) is its readout (**hidden** — the meter still drives the head / cloud / fail). Used by the two panic days, **not** lockdown.
- **KikiCloud** — [kiki_cloud.tscn](../scenes/gameplay/kiki_cloud.tscn) / [.gd](../scenes/gameplay/kiki_cloud.gd) (`class_name`): ring of `Kiki.spawn` thoughts around the head. Radius lerps `far_radius` → `near_radius` from `PanicCounter.ratio()`. Visual only (no hit, no fail). Both panic days instance it.
- **PuzzleChain + AnswerPad + ThoughtRain** — Lockdown's needs: [puzzle_chain.tscn](../scenes/gameplay/puzzle_chain.tscn) (two `WinCondition` children; last correct pad satisfies body + mind; `start()` after the setup beat; **wrong pad calls `ThoughtRain.speed_up()`**, does not fail), [answer_pad.tscn](../scenes/gameplay/answer_pad.tscn) (`class_name AnswerPad`, group `answer_pad`; stand then press `interact` / E to submit; hidden until seated), [falling_thought.tscn](../scenes/gameplay/falling_thought.tscn) + [thought_rain.gd](../scenes/gameplay/thought_rain.gd) (hit → `fail("hit")`; `autostart` off until seated; `launch_from_head` rises off the skull then rains; each miss multiplies spawn rate and fall speed by `miss_mult` 3). Setup lives on [day_lockdown.gd](../scenes/days/day_lockdown.gd) (pick up → `head.attach(body)` → bar + pedestal → `detach()` + seat). Win: `_before_head_release()` tips the pedestal +90° (clockwise, toward the exit), then `head.release()`.
- **Barbell + KikiSwarm** — Workout's needs: [barbell.tscn](../scenes/gameplay/barbell.tscn) (walk-in pickup; [barbell_frames.tres](../assets/sprites/barbell_frames.tres) `alone` / `with_body` / `lifting`; hides the body `Visual` while held; `pump()` restarts `lifting`), [kiki_swarm.tscn](../scenes/gameplay/kiki_swarm.tscn) (two `WinCondition` children; `kiki_frames.tres`; pressure 0 satisfies body+mind, 1 → `fail("kiki")`; creep after `begin()`; `StaticBody2D` circle blocks the walk to the head). Director: [day_workout.gd](../scenes/days/day_workout.gd).
- **BounceKiki** — [bounce_kiki.tscn](../scenes/gameplay/bounce_kiki.tscn) / [.gd](../scenes/gameplay/bounce_kiki.gd): `Area2D`. Stomp bounces you and emits `stomped` (the day Tweens it onto the button); any other contact is `fail("kiki")`. Hovers until stomped. Used by `platforming_day`.
- **Kiki** — [kiki.tscn](../scenes/gameplay/kiki.tscn) / [kiki.gd](../scenes/gameplay/kiki.gd) (`class_name Kiki`, an `AnimatedSprite2D`): **one intrusive thought, self-scrambling.** Every kiki picks a **random start frame** (plus a random sub-frame phase, so two on the same frame drift apart) and a **random right-angle rotation** in `_ready()`, so no two look alike (Tucker, Sat). `kind` is `lil_kiki` (9 frames) or `big_kiki` (12) — the two loops in `kiki_frames.tres`. Place it in the editor (pick `kind` in the Inspector) or spawn it from code: `Kiki.spawn(parent, &"big_kiki", Vector2(320, 90))` returns the node (`Kiki.make()` = same, no `add_child`); `scramble()` re-rolls; `scramble_on_ready = false` keeps what the editor saved. An unknown `kind` warns and falls back to lil. **Used by** [kiki_cloud.gd](../scenes/gameplay/kiki_cloud.gd). **Not yet used by** [kiki_swarm.gd](../scenes/gameplay/kiki_swarm.gd) / [flying_kiki.gd](../scenes/gameplay/flying_kiki.gd) (they build their own sprites).
- **Glasses** — [glasses.tscn](../scenes/gameplay/glasses.tscn) / [.gd](../scenes/gameplay/glasses.gd) (`class_name Glasses`): Velma pickup (`glasses.png`). Overlap satisfies **body**, emits `picked_up`, and follows the body. The day Tweens them at the head (`fly_to` → `deliver()`): **mind**, hide the sprite, `Head.set_wearing_glasses(true)`. `_ready` forces `Game.wearing_glasses = false` so an F6 on Velma still opens without them. Director: [day_velma.gd](../scenes/days/day_velma.gd) (pause, `play_throw`, knock the perch). Blur lifts on `mind` (glasses hit), not on pickup.
- **Mirror World** — two `WinCondition` children on [day_mirror.tscn](../scenes/days/day_mirror.tscn); director [day_mirror.gd](../scenes/days/day_mirror.gd) toggles `Head.look` + `Body.move_sign` + `invert_vertical`. [flying_kiki.tscn](../scenes/gameplay/flying_kiki.tscn) leaves the head (`start_head_arc`: right, down, then left; hit → `fail("kiki")`). **getting the body within `grab_radius` 56 of the glass** satisfies both (no key — Sat, Tucker); `_before_head_release()` throws the glass, then DayManager `release()`s the head after it. Reflection uses the glasses look frames when `Game.wearing_glasses`.
...
|---|---|---|---|---|---|
| [day_template.tscn](../scenes/days/day_template.tscn) | — | DayManager | body (500,320) + mind (200,320) SpatialGoals | loose at (560,306) | "Template day: touch both boxes." |
| [day_panic_still.tscn](../scenes/days/day_panic_still.tscn) + [.gd](../scenes/days/day_panic_still.gd) | [0] | DayManager | stand still until panic hits 0 (body+mind); fail = panic max, pit, or sunset; kiki cloud; head greens + shakes | loose in the tree canopy (592,246); sighs and falls off the cliff | "Hold still. Panic to zero…" |
| [day_panic.tscn](../scenes/days/day_panic.tscn) + [.gd](../scenes/days/day_panic.gd) | [2] | DayManager | **stepping on the floor button** frees the cage (body+mind, no key); fail = panic max or kiki or sunset; kiki cloud + flying kikis | `caged = true` hanging at (520,168), glasses on | "E at the button frees the cage…" |
| [day_velma.tscn](../scenes/days/day_velma.tscn) + [.gd](../scenes/days/day_velma.gd) | [4] | DayManager | climb right→centre to glasses (body); throw at perched head (mind); two small clear circles (body + head); blur lifts on hit | loose on a high right perch (592,134); body starts at (80,320) | "Climb for the glasses. Throw them at your head." |
| [platforming_day.tscn](../scenes/days/platforming_day.tscn) + [.gd](../scenes/days/platforming_day.gd) | [5] | DayManager | stomp-bounce kiki onto spike-pit button (body+mind); fail = spikes / kiki / sunset; moving platform | loose on centre perch (288,118); tilt-dumps onto the downed bridge | "Ride the moving platform. Bounce on the thought…" |
| [day_lockdown.tscn](../scenes/days/day_lockdown.tscn) | [6] | DayManager | setup (carry head to pedestal) then body+mind via PuzzleChain (4 pad puzzles, `interact`); wrong pad → faster rain; ThoughtRain hit fails; win tips the pedestal | loose in the middle (320,306), seated at (560,258) | "Go get your head." then the puzzle line |
| [day_workout.tscn](../scenes/days/day_workout.tscn) | [7] | DayManager | barbell pickup then mash `jump` vs KikiSwarm (creep-back, pumps bar); kiki circle blocks the head; fail sunset or `kiki` | loose at (560,306) | "Walk to the barbell and press E." then mash Space |
| [day_mirror.tscn](../scenes/days/day_mirror.tscn) | [8] | DayManager | jump kikis from the head (right-down-left); reaching the glass (`grab_radius` 56) throws it; look-at-glass inverts walk + jump (↓ hops); fail sunset or `kiki` | on platform at (572,258) | "Jump the thoughts. E at the glass throws it. Arrows flip with the head." |
...
Every day = `Background` (Polygon2D, sky `#988277`; *anim* adds `sky_drift.gd` on template/panic) · `Camera2D` at (320,180), fixed · floor top at **y = 320** · Body · Head · Sun · a manager · needs · an `Instruction` Label (**hidden** — `DayManager._hide_help()`). `day_template` is the copy-from reference, not in the run.

### 6.2 Transition — [scenes/transition/](../scenes/transition/)

[transition.tscn](../scenes/transition/transition.tscn) / [transition.gd](../scenes/transition/transition.gd): **one slope at 1-in-3** (−40,100)→(500,280), then a **240 px flat shelf** (500,280)→(740,280) — one `StaticBody2D` + `CollisionPolygon2D`. **The flat spot is at the far side of the screen, where the head comes to *rest*** (Tucker, Sat 2nd pass: "move flat part for the head to the far side of the screen so the unfortunate event has time to occur"). *(It was a 60 px shelf at the **top**, x=200–260, that the head **started** on — which sat it only 190 px from the body's spawn and still parked it on an incline at the other end, the very thing the shelf existed to avoid.)* The head (an `AnimatedSprite2D` on head_frames — *not* head.tscn; a frozen RigidBody2D under a PathFollow2D fights it) starts **up the slope** at (230,174) and rolls the whole hill onto the shelf, on a 3-point `Path2D`/`PathFollow2D` — (230,174)→(500,264)→(560,264), parallel to the slope then flat across the shelf, so it rides exactly 16 px above the ground the whole way (measured min clearance 15.9 px). `brake_ratio` 0.85 lands the brake on the flat — the head reaches level ground ~8 px before it starts slowing, and stops on it. The body spawns at x=50 and pulls up next to the stopped head on level ground too (`arrive_distance` 40 → x≥520, past the shelf edge). **Timeline (Sat afternoon, Tucker — the head is not fleeing; the situation is what knocks it rolling):** waits until the body is within `trigger_distance` 100 px — the "I almost caught up to you" beat — or `arrival_wait` 4 s → `_play_arrival()` (the fork's situation; the cage drops) → `head_rolled` → the head rolls (Tween on `progress`: `roll_time` 1.0 s at constant speed for `brake_ratio` 0.85 of the path, then `brake_time` 0.3 s easing out; the sprite spins whole turns over the path — rolling without slipping rounded so it stops upright) → waits for the body to reach the stopped head (`arrive_distance` 40 px; controls off via `is_scripted`, settles; `body_arrived`) for `catch_up_wait` seconds — **−1 = however long the player needs** (cage, glasses), **0 = don't wait**, which is how a fork makes its ending something you can *miss* → `hold_after_arrival` 0.6 s → **`_play_exit()`** (the second forkable beat — what happens now the head is stopped; the base does nothing) → `fade_duration` 0.4 s → `Game.next_day()`. **The body is the player's** (Sat 08:40 — body.gd untouched; the scene only sets `floor_snap_length` = `body_floor_snap` 8). ~4 s if the player runs straight; open-ended if not (`HUD/Instruction` says to go after it; Pinplaceholder copy).

**Fork recipe (decided Fri 22:50 — inherited scenes):** *Scene → New Inherited Scene* from `transition.tscn` → save as `transition_<situation>.tscn`; a script `extends "res://scenes/transition/transition.gd"` overriding only the beat it needs — `_play_arrival()` (before the roll) or `_play_exit()` (after you catch up); add props as new nodes; put it in `DAY_SCENES` right before its day. Worked example: [transition_cage.tscn](../scenes/transition/transition_cage.tscn) / [.gd](../scenes/transition/transition_cage.gd) — a second `imprisoned` / `imprisoned_glasses` sprite drops onto the resting head (glasses stay on), the knock sends the caged head tumbling down the slope **and clean off the right edge** (HeadPath curve (230,174)→(500,264)→(720,264), `roll_time` 1.6, `brake_ratio` 1.0 — it crosses the shelf and keeps going). Its `trigger_distance` is **150** (not the base's 100), so the cage lands while the body is still well back — measured: it fires 0.22 s in with the player holding forward, 149 px short, and they never touch the head (Tucker, Sat); the body runs off after it → fade → hanging `day_panic`. Comes after `day_panic_still` (cliff tip). [transition_glasses.tscn](../scenes/transition/transition_glasses.tscn) overrides `_run()`: no park; glasses arc off the right mid-roll; skull keeps going → velma (`exit_end` (720,264), 16 px above the shelf, so it now rolls off along the level ground rather than still descending). [transition_bird.tscn](../scenes/transition/transition_bird.tscn) / [.gd](../scenes/transition/transition_bird.gd) is the worked example for the *other* hook and the **first** entry in the run: `_play_arrival()` returns immediately (the intro already knocked this head loose, so nothing befalls it — it just goes as you close in) and `_play_exit()` is the whole beat — a bird dives in from off-screen left, `reparent`s the head onto itself, and carries it up and away to the right → day_panic_still, where it is stuck in a tree. It sets **`catch_up_wait` 0** and `hold_after_arrival` 0.15 so the bird runs on its own clock: measured with the player holding forward the whole way, the head stops on the shelf at 1.87 s (body 230 px back), the bird has it at 2.87 s, and the body is still **79 px short** — you arrive to watch it lift, and there is no saving it.

### 6.3 Intro, reunion, UI, placeholder

- **Intro** — [scenes/intro/intro.tscn](../scenes/intro/intro.tscn) / [.gd](../scenes/intro/intro.gd): `HeadBlob` (CharacterBody2D, `glasses_look_right` from `head_frames.tres`) runs right on its own and leaves; `_ready` sets `Game.wearing_glasses = true`. **The player has to chase it off the right edge themselves** (Sat evening: no scripting moves the body; the scene changes only once the body is `exit_margin` 24 px past the edge, then `exit_delay` 0.4 s → `Game.start_days()`). It has a **Sun** like every day (found by its `sunset` signal): sunset before you have left → `Game.restart_day()` reloads the intro. The bot's plan is `run right`. Its `@export next_scene` is **never read**. The "head pops off" beat is a TODO in the file.
- **Reunion** — [scenes/reunion/reunion.tscn](../scenes/reunion/reunion.tscn) / [.gd](../scenes/reunion/reunion.gd): camera starts at `zoom.y = -1` (screen upside down; left/right unchanged). Walk to the head — **coming within 64 px dives**, no key → dive Tween → bounce that rights the camera + body + head → `Head.attach` → scripted walk off the right toward a parked `sun.png` → fade → `next_scene` = **[scenes/main.tscn](../scenes/main.tscn)**. **There is no end card yet (N1).** [scenes/reunion/preview/sprites_preview.tscn](../scenes/reunion/preview/sprites_preview.tscn) is a standalone sprite check (ships in the export; harmless).
...
- **Sprites** — [assets/sprites/](../assets/sprites/): `body_idle/walk/throw.png` (32×32 frames) + `body_frames.tres`; `head_front/side/keyed.png` (16×16; keyed = 8 frames: look L/C/R, wink, 4-frame cage) + `head_frames.tres`; `sun.png`; `button.png` (16×16 ×3: off/mid/on, the panic floor button) + `button_frames.tres`; `tree.png` (64×96 ×4: whole → branch breaking) + `tree_frames.tres`; `bridge.png` (**24×8 tile, posts every 6 px** — a run that starts *and* ends on a post is 6n+2 wide; exported with `--crop`, not `--trim`). Sources in `src/*.aseprite`. Rule: **no generative art**; render at integer 2×. [README](../assets/sprites/README.md), [CREDITS](../assets/sprites/CREDITS.md).
...
- **Audio** — [assets/audio/README.md](../assets/audio/README.md) *(night/sfx)*: `sfx/<cue>.wav|ogg` (16 cues, WAV mono 44.1k) and `music/<track>.ogg` (`bg`, `intro`, `day`, `transition`, `reunion`, or a day's `music_track`). **`music/bg.ogg` — "dontloseyourhead" by Tucker + Ben, 3:20, Ogg Vorbis q4 stereo 48 kHz, 2.6 MB — is the one bed and it plays everywhere**, because a named track with no file falls back to it (`Music.DEFAULT_TRACK`); every scene resolving to the same name is what keeps it unbroken across scene changes. Drop in `intro.ogg` and the intro crossfades to it with no code change. `Sfx` prints the missing-cue list at boot (`Sfx: 16/16 cues have no file yet — …`); Music reports nothing.
