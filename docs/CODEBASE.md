...
```mermaid
flowchart LR
  I["intro.tscn<br/>(glasses on)"] -- "Game.start_days()" --> T1["transition_cage.tscn<br/>(roll, cage drops)"]
...
- **PanicCounter** — [panic_counter.tscn](../scenes/gameplay/panic_counter.tscn) / [.gd](../scenes/gameplay/panic_counter.gd) (`class_name`): panic starts at 15, rises while the body moves, holds, then falls when still; **max fails** the day (`DayManager.fail("panic")`). **Does not win the day** (`win_on_zero` defaults false — the release button does). Drives `head.set_agitation()`; signals `panic_changed(int)`, `calmed`; group `panic_counter`. [panic_label.gd](../scenes/gameplay/panic_label.gd) is its readout.
...
- **Kiki** — [kiki.tscn](../scenes/gameplay/kiki.tscn) / [kiki.gd](../scenes/gameplay/kiki.gd) (`class_name Kiki`, an `AnimatedSprite2D`): **one intrusive thought, self-scrambling.** Every kiki picks a **random start frame** (plus a random sub-frame phase, so two on the same frame drift apart) and a **random right-angle rotation** in `_ready()`, so no two look alike (Tucker, Sat). `kind` is `lil_kiki` (9 frames) or `big_kiki` (12) — the two loops in `kiki_frames.tres`. Place it in the editor (pick `kind` in the Inspector) or spawn it from code: `Kiki.spawn(parent, &"big_kiki", Vector2(320, 90))` returns the node (`Kiki.make()` = same, no `add_child`); `scramble()` re-rolls; `scramble_on_ready = false` keeps what the editor saved. An unknown `kind` warns and falls back to lil. **Every kiki in the game comes from here** (Sat, Tucker): [kiki_swarm.gd](../scenes/gameplay/kiki_swarm.gd) spawns its ring with `Kiki.spawn()`, [flying_kiki.tscn](../scenes/gameplay/flying_kiki.tscn)'s `$Visual` carries the script, and [falling_thought.tscn](../scenes/gameplay/falling_thought.tscn)'s brown placeholder rectangle is now a kiki too — so the lockdown day rains actual intrusive thoughts.
...
| [day_panic.tscn](../scenes/days/day_panic.tscn) + [.gd](../scenes/days/day_panic.gd) | [1] | DayManager | **stepping on the floor button** frees the cage (body+mind, no key); fail = panic max or kiki or sunset | `caged = true` hanging at (520,168), glasses on | "E at the button frees the cage…" |
| [day_velma.tscn](../scenes/days/day_velma.tscn) | [3] | DayManager | climb later platforms to `glasses.png`, carry back to the head | loose near entrance (180,306), no glasses until delivery | "Climb up for the glasses, then bring them to your head." |
...
| [day_lockdown.tscn](../scenes/days/day_lockdown.tscn) | [5] | DayManager | setup (walk to the head → it is picked up; stand on the pedestal → it is seated) then body+mind via PuzzleChain (4 puzzles, **hop onto the right plate**); ThoughtRain fails on hit (fast, burst 2); win tips the pedestal | loose in the middle (320,306), seated at (560,258) | "Walk to the head and press E." then the puzzle line |
| [day_workout.tscn](../scenes/days/day_workout.tscn) | [6] | DayManager | walk into the barbell to pick it up, then mash `jump` vs KikiSwarm (creep-back, pumps bar); kiki circle blocks the head; fail sunset or `kiki` | loose at (560,306) | "Walk to the barbell and press E." then mash Space |
| [day_mirror.tscn](../scenes/days/day_mirror.tscn) | [7] | DayManager | jump kikis from the glass; **reaching the glass (`grab_radius` 56) throws it**, no key; look-at-glass inverts walk + jump (↓ hops); fail sunset or `kiki` | on platform at (572,258) | "Jump the thoughts. E at the glass throws it. Arrows flip with the head." |
