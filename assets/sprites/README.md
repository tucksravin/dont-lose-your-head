...
- `bridge.png` — a fence/railing, **26×8**, recoloured Fri 21:5x at Tucker's request: posts `#006a3d` (ground green), rope + deck line `#45381c` (dark brown). They were `#45381c` / `#988277` before, and `#988277` is the sky colour, so the rope and deck line were invisible against the sky. The palette was swapped **in the `.aseprite` source**, so re-exporting keeps the new colours. Used tiled (via `texture_repeat` + `region_rect`) for both the bridge and the three steps in `platforming_day` (trimmed Fri 21:10: the canvas was 64×16 with 34 px of empty space on the right and 8 px on top, which put a gap around it wherever you placed it). At the project's integer scale 2 it draws 52×16.
