...
**Kikis (Sat, for Sean):** `kikis.png` — 320×16, **20 frames of 16×16**, the intrusive-thought shapes, in the violet ramp (`#5e2d8c` / `#8a4fb5` / `#c79df2`) only. Source `src/kikis.aseprite` (tags: **`lil_kiki`** = f0–8, the small one, ~4×4 px; f9–19 are a bigger one, **untagged in the source**). Drop-in: **`kiki_frames.tres`** (SpriteFrames) → an `AnimatedSprite2D` with `sprite_frames = res://assets/sprites/kiki_frames.tres`, `animation = "lil_kiki"` (9 frames, 10 fps, loops — the file's 100 ms/frame) or `"big_kiki"` (f9–19, 10 fps, loops; *that name is the agent's placeholder — tag it in Aseprite and rename the animation when it has one*). Render at 2× like everything else (→ 32×32 cells; the lil kiki is ~8 px on screen). No collision shape ships with it — what a kiki *does* is a day decision.
