# Palette — Gooseberry Ghost (+ bone shadow)

`gooseberry-ghost-plus-bone-shadow.gpl` — load it in **Aseprite**: Palette menu (the ▸ beside the swatches) → *Load Palette…* → pick this file. Or drag `…-.png` onto the palette bar; it reads the swatches straight off the image.

`gooseberry-ghost-plus-bone-shadow.png` — 288×32, one 32×32 swatch per colour, in the order below. Handy for the itch page and for eyedropping in any editor.

| Hex | Use so far |
|---|---|
| `#988277` | sky / background mauve |
| `#645543` | mid brown |
| `#45381c` | dark brown |
| `#201c02` | outline (near-black) |
| `#f1ffaf` | bone (body), sun disc |
| `#cdcd99` | bone shadow — far-side limbs (added by us, not in the original 8) |
| `#b2f167` | light green, sun rays |
| `#25c04b` | mid green |
| `#006a3d` | dark green / ground |

Base palette: **Gooseberry Ghost** by Rustocrat (Lospec), 8 colours; `#cdcd99` is ours. Everything shipped should come from this list — if you need a new colour, add it here and say so, don't one-off it in a scene.
