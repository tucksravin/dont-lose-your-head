...
## `Background` Polygon2D; it finds the scene's Sun and lerps the node's own
## `color` (that IS the dawn colour — set it in the Inspector as usual) toward
## `dusk` by Sun.progress(). Deliberately off-palette in between (Tucker OK'd
## the drift Fri night): the two ends are palette stops, the journey isn't.
## Set `steps` > 0 to quantise the drift into that many plateaus instead of a
## smooth ramp.
