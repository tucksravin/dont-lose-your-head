...
4. You should land on the **landing screen** — *"Let the Intrusive Thoughts In"* — with the guy walking in place under it. Clicking it (or Enter/Space) plays the **intro**, a cutscene with no input: he stops by a slope, intrusive thoughts wind his head up until it pops off and rolls away. Then the run starts, and the first scene you actually drive (`A/D` move, `Space` jump) is the chase down the hill, which ends with a bird carrying the head off. `F9` shows the debug overlay, `F7` skips to the next scene (debug builds only). The old "template OK — press a key" screen is `scenes/main.tscn`, now only the placeholder after the reunion.
